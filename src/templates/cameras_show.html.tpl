...
{% block content %}
    <div class="cameras single reload" data-timeout="60000">
        ...
        {% else %}
            <img class="single" alt="{{ camera.id }}" height="{{ camera.height }}" width="{{ camera.width }}" src="{{ camera.protocol }}://{{ camera.username }}:{{ camera.password }}@{{ camera.url }}/axis-cgi/mjpg/video.cgi?camera={{ camera.camera | default(1) }}&compression={{ camera.compression | default(10) }}&fps={{ camera.fps | default(10) }}&clock={{ camera.clock | default(0) }}" />
        {% endif %}
    </div>
{% endblock %}
