...
TYPE_NAMES = {USER_TYPE: "user", SUPER_USER_TYPE: "super user", ADMIN_TYPE: "admin"}
...
class Account(base.Base):
    ...
    username = dict(index=True, immutable=True, default=True)

    password = dict(private=True)

    confirmation = dict(private=True)

    email = dict(index=True, immutable=True)

    login_count = dict(type=int)

    last_login = dict(type=float)

    type = dict(type=int)

    tokens = dict(type=list)

    cameras = dict(type=quorum.references(camera.Camera, name="camera_id"))
    ...
    def login(cls, username, password):
        ...
        if not username or not password:
            raise quorum.OperationalError(
                "Both username and password must be provided", code=400
            )
        ...
        if not account:
            raise quorum.OperationalError("No valid account found", code=403)
        ...
        if not password_sha1 == _password:
            raise quorum.OperationalError("Invalid or mismatch password", code=403)
    ...
    def confirmed(cls, confirmation):
        ...
        if not account:
            raise quorum.OperationalError("Account not found or invalid confirmation")
        if account.enabled:
            raise quorum.OperationalError("Account is already active")
    ...
    def _build(cls, model, map):
        ...
        last_login_string = last_login_date and last_login_date.strftime(
            "%d/%m/%Y %H:%M:%S"
        )
    ...
    def pre_create(self):
        ...
        if self.type in (SUPER_USER_TYPE, ADMIN_TYPE):
            self.cameras = None
        ...
        self.password = hashlib.sha1(
            quorum.legacy.bytes(self.password + PASSWORD_SALT)
        ).hexdigest()
        self.api_key = hashlib.sha1(quorum.legacy.bytes(str(uuid.uuid4()))).hexdigest()
        self.confirmation = hashlib.sha1(
            quorum.legacy.bytes(str(uuid.uuid4()))
        ).hexdigest()
    ...
    def pre_update(self):
        ...
        if has_password and self.password == "":
            del self.password
