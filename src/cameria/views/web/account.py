...
def list_accounts():
    return flask.render_template(
        "account/list.html.tpl", link="accounts", sub_link="list"
    )
...
def list_accounts_json():
    object = quorum.get_object(alias=True, find=True, sort=[("username", 1)])
...
def show_account(username):
    ...
    return flask.render_template(
        "account/show.html.tpl", link="accounts", sub_link="show", account=account
    )
...
def update_account(username):
    ...
    # was just updated
    return flask.redirect(flask.url_for("show_account", username=username))
