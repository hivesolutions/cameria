...
def list_cameras():
    return flask.render_template(
        "camera/list.html.tpl", link="cameras", sub_link="list"
    )
...
def list_cameras_json():
    object = quorum.get_object(alias=True, find=True, sort=[("camera_id", 1)])
...
def create_camera():
    ...
    return flask.redirect(flask.url_for("show_camera", camera_id=camera.camera_id))
...
def show_camera(camera_id):
    ...
    return flask.render_template(
        "camera/show.html.tpl", link="cameras", sub_link="show", camera=camera
    )
...
def update_camera(camera_id):
    ...
    # was just updated
    return flask.redirect(flask.url_for("show_camera", camera_id=camera_id))
...
def delete_camera(camera_id):
    camera = models.Camera.get_a(camera_id=camera_id)
    camera.delete()
    return flask.redirect(flask.url_for("list_cameras"))
...
def settings_camera(camera_id):
    camera = models.Camera.get_a(camera_id=camera_id)
    return flask.render_template(
        "camera/settings.html.tpl", link="cameras", sub_link="settings", camera=camera
    )
