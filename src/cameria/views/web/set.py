...
def list_sets():
    return flask.render_template("set/list.html.tpl", link="sets", sub_link="list")


@app.route("/sets.json", methods=("GET",), json=True)
@quorum.ensure("sets.list", json=True)
def list_sets_json():
    object = quorum.get_object(alias=True, find=True, sort=[("name", 1)])
...
def create_set():
    ...
    return flask.redirect(flask.url_for("show_set", set_id=set.set_id))
...
def show_set(set_id):
    ...
    set.merge_cameras()
    return flask.render_template(
        "set/show.html.tpl", link="sets", sub_link="show", set=set
    )
...
def edit_set(set_id):
    set = models.Set.get_a(set_id=set_id)
    return flask.render_template(
        "set/edit.html.tpl", link="sets", sub_link="edit", set=set, errors={}
    )
...
def update_set(set_id):
    ...
    # redirects the user to the show page of the set that
    # was just updated
    return flask.redirect(flask.url_for("show_set", set_id=set_id))
...
def delete_set(set_id):
    set = models.Set.get_a(set_id=set_id)
    set.delete()
    return flask.redirect(flask.url_for("list_sets"))
...
def settings_set(set_id):
    set = models.Set.get_a(set_id=set_id)
    return flask.render_template(
        "set/settings.html.tpl", link="sets", sub_link="settings", set=set
    )
